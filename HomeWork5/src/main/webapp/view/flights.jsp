      ...
      <form action="${pageContext.request.contextPath}/flights" id="my_post" method="POST">
         ...
         <select name ="airportDeparture">
            <c:forEach var="item" items="${listAirports}">
               <option value=${item.airportCode}
               <c:if test="${item.airportCode == airportDeparture}"> selected </c:if>
               >${item.city} - ${item.airportCode}</option>
            </c:forEach>
         </select>
         ...
         <select name="airportArrival">
            <c:forEach var="item" items="${listAirports}">
               <option value=${item.airportCode}
               <c:if test="${item.airportCode == airportArrival}"> selected </c:if>
               >${item.city} - ${item.airportCode}</option>
            </c:forEach>
         </select>
